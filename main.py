...
list =['*','*','*','*','*','W','e','l','c','o','m','e',' ','T','o',' ','I','n','s','t','a','B','o','t',' ','A','p','p','l','i','c','a','t','i','o','n','*','*','*','*','*']
i=0
print "\n\n\n\t\t\t\t\t\t\t\t\t\t\t\t",
while i<len(list):
    if(i<5) or i>35:
        color_text = colored(list[i],"red")             #magical coding for displaying 1st line of my application
        print color_text,
    if (i > 4) and i<36:
        color_text = colored(list[i], "green")  # magical coding for displaying 1st line of my application
        print color_text,
    time.sleep(.22)
    i=i+1
print"welcome"
...
menu_choices = "What do you want to do. Enter your choice" \
               "\n\n\t\t1. Profile Info " \
               "\n\t\t2. Download Own Post " \
...
while show_menu:        #if user enter valid key
    ...
    if (menu_choice) > 0:
        ...
        if menu_choice == 1:
            ...
            searching()
            insta_username = raw_input("\nEnter The Username Who's Information You Want.(eg. khiladimanu1)\n")
